...
def test_storage():
    ...
    storage = Storage(base_url=base_url,
                      # dataset_id='test-dataset-010203',
                      dataset_id='newcastle-city-council-payments-over-500',
                      # dataset_id='us-national-foreclosure-statistics-january-2012',
                      api_key='env:CKAN_API_KEY')

    bucket_list = storage.buckets
    print(bucket_list)
    # for b in bucket_list:
    #     print(storage.describe(bucket=b))

    bucket_iter = storage.iter(bucket_list[0])
    print(next(bucket_iter))
    bucket_iter = storage.iter(bucket_list[1])
    print(next(bucket_iter))
    bucket_iter = storage.iter(bucket_list[2])
    print(next(bucket_iter))
